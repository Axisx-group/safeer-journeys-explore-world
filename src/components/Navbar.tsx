...
const Navbar = () => {
  ...
  return (
    <>
      {/* Main Navigation - Fixed positioning with responsive width */}
      <nav className="fixed top-0 left-0 right-0 bg-gradient-to-r from-white via-gray-50 to-white shadow-xl z-50 border-b border-gray-200/50 backdrop-blur-lg w-full">
        ...
        {isMenuOpen && (
          <div className="md:hidden bg-white/98 backdrop-blur-lg border-t border-gray-200/50 shadow-lg w-full">
              ...
              {navigationItems.map((item, index) => {
                ...
                return (
                  <Link
                    key={item.href}
                    to={item.href}
                    className={`flex items-center px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-300 w-full ${
                      isActive 
